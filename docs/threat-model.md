...
| Audit logs | Hash-chained SQLite audit records | Tamper-evident when the hash secret and any external anchor are protected; not writable by agents |
...
| Audit log writer unavailable | Deny before execution; after a connector call, retain the terminal idempotency result to avoid duplicate financial side effects. |
...
| `decision.final` | Final guardrail decision returned for the intent | intentId, outcome, reasons, requiresHumanApproval, decision |
...
| `broker.failed` | Broker execution or broker-side rejection failed closed | intentId, rejectionReason or error |
...
The audit event schema currently includes these event types: `intent.received`, `intent.validated`, `intent.rejected`, `reviewer.started`, `reviewer.completed`, `reviewer.failed`, `policy.evaluated`, `policy.failed`, `risk.evaluated`, `risk.failed`, `decision.final`, `approval.requested`, `approval.approved`, `approval.denied`, `approval.timeout`, `allowlist.updated`, `signer.requested`, `signer.completed`, `signer.failed`, `broker.revalidated`, `broker.executed`, `broker.failed`, `killswitch.activated`, and `killswitch.blocked`.

All audit events include timestamp, correlationId, environment, and a hash linking to the previous event. Tamper evidence depends on protecting `AUDIT_HASH_SECRET` and, outside dev/test, the configured external hash anchor.
...
| `canary_live` | Limited live trading with tight limits | Real execution with canary notional limits when explicitly enabled | Required unless explicitly disabled by policy for low-risk allowlisted actions |
| `production` | Reserved/planned profile | Broker execution currently rejects production; production limits are policy placeholders | Required if/when production execution is implemented |
